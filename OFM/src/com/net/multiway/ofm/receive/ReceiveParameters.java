package com.net.multiway.ofm.receive;
...
import com.net.multiway.ofm.model.Package;
...
public class ReceiveParameters extends Package {
    ...
    public void parserPartB() throws IOException {
        ...
        List<DataGraphic> dt = new ArrayList<>();
        byte[] b = new byte[2];

        for (int i = 0; i < this.length; i++) {
            in.read(b);
            dt.add(new DataGraphic(Utils.byte2ToInt(b)));
        }
        data.setDataGraphicList(dt);
    }
    ...
    public void printPartC(PrintWriter out) {
        List<DataEvent> ar = data.getDataEventList();
        out.println("/**************************************************************/");
        out.println("/***********************OUTPUT EVENTS**************************/");
        out.println("/**************************************************************/");
        int i = 1;
        for (DataEvent data : ar) {
            out.println("EVENT " + i++);
